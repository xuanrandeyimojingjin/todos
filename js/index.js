$(function(){
    ...
    var render=function(){
        ...
        for(var i=0;i<database.length;i++){
            var v=database[i];
            $('<li  data-id="'+ v.id+'"  class="inner '+(v.isDone?'wangcheng':'')+'"> <div class="check">✓</div> <p>'+ v.name+'</p> <input type="text" value="'+ v.name+'"> <div class="delete">⨉</div> </li>').appendTo('.content');
        }
    }
    ...
    $('.tobo .footer .inner .box').on('click',function () {
        render();
    })
    $('.tobo .footer .inner .active').on('click',function () {
        render();
        var li = database;
        if( li.hasClass('wangcheng')){
            li.css({
